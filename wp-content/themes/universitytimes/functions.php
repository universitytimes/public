...
function wpse49339_enqueue_script() {
    wp_enqueue_script( 'customajax.js', get_template_directory_uri().'/library/js/customajax.js', array( 'jquery'  ) );
}
add_action( 'wp_enqueue_scripts', 'wpse49339_enqueue_script' );





function wpse49339_localize_script() {
     wp_localize_script( 'customajax.js', 'ajax_object', array('ajax_url' => admin_url('admin-ajax.php')) );
}
add_action( 'wp_enqueue_scripts', 'wpse49339_localize_script' );
...
add_action('woo_init','woo_get_most_popular_list_schedule');
add_action('woo_get_most_popular_list_job','woo_get_most_popular_list_function');
...
function wpse_ad_content($content)
{
    ...
    for ($i = 0; $i < count($content); $i++) {
        if ($i == $paragraphAfter) {
            $new_content.= '<div class="mobilearticlead">ADVERTISEMENT<a href="mailto:editor@universitytimes.ie">	

	<img src="https://universitytimes.ie/wp-content/themes/universitytimes/library/images/utad.jpg" />
...
function woo_get_most_popular_list_schedule(){
  ...
  if(!wp_next_scheduled('woo_get_most_popular_list_job'))
   ...
   wp_schedule_event (time(), 'hourly', 'woo_get_most_popular_list_job');
}



function woo_get_most_popular_list_function() {
				...
				if ( $the_popular_query->have_posts() ) :  $count = 0; 
	
					$popularlistarray = array();
	
					while ( $the_popular_query->have_posts() ) : $the_popular_query->the_post(); 

							$permalink = get_permalink();		 
													 
									$facebookurl = "http://api.facebook.com/restserver.php?method=links.getStats&urls=" . $permalink;
					
									$facebookurl2 = "http://api.facebook.com/restserver.php?method=links.getStats&urls=" . "https://universitytimes.ie/?p=" . get_the_ID();
									...
									$facebookdata2 = simplexml_load_file($facebookurl3);
						
									$facebook_total_engagement2 = $facebookdata3->link_stat->total_count;
									...
									$popularlistarray[$postid] = $facebook_total_engagement;
...
wp_reset_query(); 
			...
			arsort($popularlistarray);
			
			
			$finalpopularlistarray = array_keys($popularlistarray);	
			
	
	
	
	update_option( 'mostpopulararticles', $finalpopularlistarray); 
...
/* DON'T DELETE THIS CLOSING TAG */ ?><?php
...
function custom_headshot_meta_box() {
	...
	add_meta_box( 'tagsdiv-normalheadshot', 'Standard Headshot', 'normalheadshot_meta_box', 'by', 'side', 'high' );
...
function custom_columnheadshot_meta_box() {
	...
	add_meta_box( 'tagsdiv-columnheadshot', 'Column Headshot', 'columnheadshot_meta_box', 'by', 'side', 'high' );
...
//first do the translations part for GUI
  ...
  $labels = array(
    'name' => _x( 'Story Topic', 'taxonomy general name' ),
    'singular_name' => _x( 'Story Topic', 'taxonomy singular name' ),
    'search_items' =>  __( 'Search Story Topics' ),
    'all_items' => __( 'All Story Topics' ),
    'parent_item' => __( 'Parent Story Topic' ),
    'parent_item_colon' => __( 'Parent Story Topic:' ),
    'edit_item' => __( 'Edit Story Topic' ), 
    'update_item' => __( 'Update Story Topic' ),
    'add_new_item' => __( 'Add New Story Topic' ),
    'new_item_name' => __( 'New Story Topic Name' ),
    'menu_name' => __( 'Story Topics' ),
  ); 	

// Now register the taxonomy

  register_taxonomy('storytopics',array('post'), array(
    'hierarchical' => true,
    'labels' => $labels,
    'show_ui' => true,
...
?><?php
	...
	add_action('save_post', 'ut_save_custom_social_meta', 10, 2);

function ut_save_custom_social_meta($id, $post) {
    ...
    $ut_custom_social_headline = $_POST['custom_social_headline'];

    // update post meta as you wish
    
    if ( $ut_custom_social_headline !== '' && $ut_custom_social_headline !== false ) {
    
    update_post_meta( $post->ID, 'definedsocialtitle', $ut_custom_social_headline);
	    ...
	    delete_post_meta( $post->ID, 'definedsocialtitle' );
...
add_action('save_post', 'ut_save_bottom_of_post_meta', 10, 2);

function ut_save_bottom_of_post_meta($id, $post) {
    ...
    $ut_bottom_of_post = $_POST['bottom_of_post'];

    // update post meta as you wish
    
    if ( $ut_bottom_of_post !== '' && $ut_bottom_of_post !== false ) {
    
    update_post_meta( $post->ID, 'bottomofpost', $ut_bottom_of_post);
	    ...
	    delete_post_meta( $post->ID, 'bottomofpost' );
...
add_action('save_post', 'ut_save_custom_info_deck', 10, 2);

function ut_save_custom_info_deck($id, $post) {
    ...
    $ut_custom_info_deck = $_POST['custom_info_deck'];

    // update post meta as you wish
    
    if ( $ut_custom_info_deck !== '' && $ut_custom_info_deck !== false ) {
    
    update_post_meta( $post->ID, 'custominfodeck', $ut_custom_info_deck);
	    ...
	    delete_post_meta( $post->ID, 'custominfodeck' );
	...
	function custom_social_meta_box() {

	
	add_meta_box( 'tagsdiv-customsocial', 'Custom Social Meta', 'custom_social_section_meta_box', 'post', 'normal', 'default' );
	
	

}
add_action('add_meta_boxes', 'custom_social_meta_box');

/* Prints the taxonomy box content */
function custom_social_section_meta_box($post) {  
	
	
	$current_defined_social_title = htmlspecialchars(get_post_meta( $post->ID, 'definedsocialtitle', true));
...
?>
    ...
    <label for="custom_social_headline">Custom social headline:</label> <input type="text" name="custom_social_headline" value="<?php 
	    
	    if ( ! empty( $current_defined_social_title ) ) {
    echo $current_defined_social_title;
}
...
<?php }
	...
	function custom_bottom_meta_box() {

	
	add_meta_box( 'tagsdiv-custombottom', 'Bottom of Post', 'custom_bottom_section_meta_box', 'post', 'advanced', 'default' );
	
	

}
add_action('add_meta_boxes', 'custom_bottom_meta_box');

/* Prints the taxonomy box content */
function custom_bottom_section_meta_box($post) {  
	
	
	$current_bottom_of_post = htmlspecialchars(get_post_meta( $post->ID, 'bottomofpost', true));
...
<textarea name="bottom_of_post" rows="3" style="width: 100%; font-family: Consolas, Monaco, monospace;
font-size: 13px;
font-style: normal; padding: 10px; line-height: 19.5px;"><?php 	if ( ! empty( $current_bottom_of_post ) ) {
    echo $current_bottom_of_post;
}
...
<?php }
	...
	function custom_info_deck_box() {

	
	add_meta_box( 'tagsdiv-infodeck', 'Info Deck', 'custom_info_deck_section_meta_box', 'post', 'normal', 'default' );
	
	

}
add_action('add_meta_boxes', 'custom_info_deck_box');

/* Prints the taxonomy box content */
function custom_info_deck_section_meta_box($post) {  
	
	
	$current_info_deck = htmlspecialchars(get_post_meta( $post->ID, 'custominfodeck', true));
...
<textarea name="custom_info_deck" rows="3" placeholder="This text appears in some templates in place of a sub-heading" style="width: 100%; font-family: Consolas, Monaco, monospace;
font-size: 13px;
font-style: normal; padding: 10px; line-height: 19.5px;"><?php 	if ( ! empty( $current_info_deck ) ) {
    echo $current_info_deck;
}
...
<div class="tagsdiv" id="<?php echo $tax_name; ?>">
	...
	if ($section_IDs[0] == 1781 || $section_IDs[0] == 1782) {
  ...
  <?php
	...
	$opinionterm_id = $opinionterm->term_id;
		...
		$infocusterm = get_term_by( 'slug', 'infocus', 'section' );
	$infocusterm_id = $infocusterm->term_id;


	$radiusterm = get_term_by( 'slug', 'radius', 'section' );
	$radiusterm_id = $radiusterm->term_id;
	
	$magazineterm = get_term_by( 'slug', 'magazine', 'section' );
	$magazineterm_id = $magazineterm->term_id;
	
	$blogsterm = get_term_by( 'slug', 'blogs', 'section' );
	$blogsterm_id = $blogsterm->term_id;
	
	$freshersterm = get_term_by( 'slug', 'freshers', 'section' );
	$freshersterm_id = $freshersterm->term_id;
	
	$liveblogsterm = get_term_by( 'slug', 'live', 'articletype' );
	$liveblogsterm_id = $liveblogsterm->term_id;
...
?>
        ...
        jQuery("#thisselectionselector").bind('change', function () {


           var newstermid = "<?php echo $newsterm_id; ?>";
           var opiniontermid = "<?php echo $opinionterm_id; ?>";
           var infocustermid = "<?php echo $infocusterm_id; ?>";
           var radiustermid = "<?php echo $radiusterm_id; ?>";
           var magazinetermid = "<?php echo $magazineterm_id; ?>";
           var blogstermid = "<?php echo $blogsterm_id; ?>";
           var fresherstermid = "<?php echo $freshersterm_id; ?>";
           var liveblogstermid = "<?php echo $liveblogsterm_id; ?>";
    ...
    str = parseInt(jQuery("#section option:selected").val());
        ...
        if(str == newstermid){
  ...
  else if(str == blogstermid){

         jQuery("#posttypeselector").show();



		 jQuery("#blogswoo").show();
		 
		 jQuery("#the_subtitle").hide();
		 
		  jQuery("#the_subtitle").val('');
         ...
         jQuery("#typeteller").val('magazine');
         ...
         jQuery("#typeteller").val('freshers');
...
jQuery('#thisselectionselector').trigger('change');
   ...
   <div id="newswoo">
       ...
       <select id="news_articletype" name="news_articletype" class='postform' >
	...
	?>>News Feature</option>
	 ...
	 <option class="level-0" value="live" <?php
	if ( is_object_in_term( $post->ID, 'articletype', 'live' ) ) :
		echo 'selected="selected"';
	else :
		echo '';
	endif;
	?>>Live Blog</option>
     ...
     <div id="blogswoo">
       ...
       <select name="blogs_articletype" class='postform' >

           <option class="level-0" value="-1">Select blog</option>

        <option class="level-0" value="highereducation" <?php
	if ( is_object_in_term( $post->ID, 'section', 'highereducation' ) ) :
		echo 'selected="selected"';
	else :
		echo '';
	endif;
	?>>Higher Education Blog</option>
    <option class="level-0" value="inthemargins" <?php
	if ( is_object_in_term( $post->ID, 'section', 'inthemargins' ) ) :
		echo 'selected="selected"';
	else :
		echo '';
	endif;
	?>>In the Margins</option>
</select>
   ...
   <div id="opinionwoo">
	...
	<option class="level-0" value="irishopinioncontrib" <?php
	if ( is_object_in_term( $post->ID, 'articletype', 'irishopinioncontrib' ) ) :
		echo 'selected="selected"';
	else :
		echo '';
	endif;
	?>>Irish Opinion Contribution</option>
...
<option class="level-0" value="analysis-2" <?php
    ...
    <option class="level-0" value="columns" <?php
...
<option class="level-0" value="editorials" <?php
	...
	<option class="level-0" value="eagarfhocail" <?php
	if ( is_object_in_term( $post->ID, 'articletype', 'eagarfhocail' ) ) :
		echo 'selected="selected"';
	else :
		echo '';
	endif;
	?>>Irish Editorial</option>
	
	
	<option class="level-0" value="editorialnotebook" <?php
	if ( is_object_in_term( $post->ID, 'articletype', 'editorialnotebook' ) ) :
		echo 'selected="selected"';
	else :
		echo '';
	endif;
	?>>Editorial Notebook</option>
...
<option class="level-0" value="op-ed" <?php
	...
	<option class="level-0" value="profile" <?php
	if ( is_object_in_term( $post->ID, 'articletype', 'profile' ) ) :
		echo 'selected="selected"';
	else :
		echo '';
	endif;
	...
	<option class="level-0" value="loveinterestcolumn" <?php
	if ( is_object_in_term( $post->ID, 'articletype', 'loveinterestcolumn' ) ) :
		echo 'selected="selected"';
	else :
		echo '';
	endif;
	?>>Love Interest Column</option>
...
<option class="level-0" value="radius5of" <?php
	...
	?>>5 of...</option>
       ...
       <select name="magazine_articletype" class='postform' >

           <option class="level-0" value="-1">Select article type</option>

        <option class="level-0" value="magazinefeature" <?php
	if ( is_object_in_term( $post->ID, 'articletype', 'magazinefeature' ) ) :
		echo 'selected="selected"';
	else :
		echo '';
	endif;
	?>>Feature</option>
    <option class="level-0" value="magazinearticle" <?php
	if ( is_object_in_term( $post->ID, 'articletype', 'magazinearticle' ) ) :
		echo 'selected="selected"';
	else :
		echo '';
	endif;
   ...
   <div id="sportswoo">SPORTS WOO!</div>
    ...
    <div id="liveblogswoo" style="display: none;">
	
	<label style="margin: 10px 0px 10px 0px; display: block;" for="livebloghometick">

    <input style="margin-right: 2px;" type="checkbox" id="livebloghometick" name="livebloghometick" value="yes"> Show on top of homepage</label>
...
function section_save_postdata( $post_id ) {
	...
	if($typeteller == 'blogs'){
		...
		$section = $_POST['blogs_articletype'];
...
function articletype_save_postdata( $post_id ) {
	...
	elseif($typeteller == 'radius'){
		...
		elseif($typeteller == 'magazine'){

		$articletype_ID = $_POST['magazine_articletype'];


	}
	
	
	elseif($typeteller == 'blogs'){

		$articletype_ID = $_POST['blogs_articletype'];
	...
	if ($articletype_ID != -1 || $articletype_ID != 'inthemargins' || $articletype_ID != 'highereducation') {
...
function ut_radius_date_meta_box() {
	...
	add_meta_box( 'tagsdiv-utradiusdate', 'Event Details', 'utradiusdate_meta_box', 'ut_radius_event_type', 'side', 'high' );
...
?>
 ...
 <?php echo '<input type="hidden" name="ut_utradiusdate_nonce" id="ut_utradiusdate_nonce" value="' .
	...
	?>> Event occurs on multiple days</label></div>
...
<div id="eventdatewrap_until" style="
    ...
    <span style="display:block; margin-top: 5px;">Last date:</span>
...
" size="4" maxlength="4" autocomplete="off" /> @ <label for="hour_hh_until" class="screen-reader-text">Hour</label>



<input style="font-size: 12px; width: 28px; height: 28px; padding: 2px;

<?php $utradiusdate_hour_until = get_post_meta($post->ID, 'utradiusdate_hour_until', true);

	if ($utradiusdate_hour_until == 'hh') { echo 'color: red; font-weight: bold;';}   ?>

" type="text" id="hour_hh_until" name="hour_hh_until" value="

<?php echo $utradiusdate_hour_until; ?>

" size="2" maxlength="2" autocomplete="off" /> :



<label for="minute_mm_until" class="screen-reader-text">Minute</label>
<input style="font-size: 12px; width: 30px; height: 28px; padding: 2px;


<?php $utradiusdate_minute_until = get_post_meta($post->ID, 'utradiusdate_minute_until', true);

	if ($utradiusdate_minute_until == 'mm') { echo 'color: red; font-weight: bold;';}   ?>




" type="text" id="minute_mm_until" name="minute_mm_until" value="

<?php echo $utradiusdate_minute_until ?>
...
<?php $utradiusevent_time_description = get_post_meta($post->ID, 'utradiusevent_time_description', true);
...
<div class="fieldinmeta" ><label for="utradiusevent_time_description" style="display: block; margin-top: 8px;">Word description of event time (if it's on at multiple times):</label> <input type="text" name="utradiusevent_time_description" id="utradiusevent_time_description" value="<?php echo $utradiusevent_time_description; ?>" /></div>

<div class="fieldinmeta" ><label for="utradiusevent_tickets_url" style="display: block; margin-top: 8px;">Tickets URL:</label> <input type="text" name="utradiusevent_tickets_url" id="utradiusevent_tickets_url" value="<?php echo $utradiusevent_tickets_url; ?>" /></div>
...
function save_utradiusdateurl( $post_id )
{
	...
	update_post_meta($post_id, 'utradiusevent_time_description', $_POST['utradiusevent_time_description']);
	
	update_post_meta($post_id, 'utradiusevent_tickets_url', $_POST['utradiusevent_tickets_url']);
	...
	if ($_POST['radius_multi_day_event'] == 'yes' ) {
		...
		if (ctype_digit($_POST['year_yyyy_until']) && $_POST['year_yyyy_until'] >= $_POST['year_yyyy']) {
			...
			(($_POST['hour_hh_until'] === '00') or
				($_POST['hour_hh_until'] === '01') or
				($_POST['hour_hh_until'] === '02') or
				($_POST['hour_hh_until'] === '03') or
				($_POST['hour_hh_until'] === '04') or
				($_POST['hour_hh_until'] === '05') or
				($_POST['hour_hh_until'] === '06') or
				($_POST['hour_hh_until'] === '07') or
				($_POST['hour_hh_until'] === '08') or
				($_POST['hour_hh_until'] === '09'))


			or


			( ctype_digit($_POST['hour_hh_until']) && $_POST['hour_hh_until'] > 9 && $_POST['hour_hh_until'] < 24)) {


			update_post_meta( $post_id, 'utradiusdate_hour_until', $_POST['hour_hh_until'] );
			...
			update_post_meta( $post_id, 'utradiusdate_hour_until', 'hh' );
		...
		if (
			...
			(($_POST['minute_mm_until'] === '00') or
				($_POST['minute_mm_until'] === '01') or
				($_POST['minute_mm_until'] === '02') or
				($_POST['minute_mm_until'] === '03') or
				($_POST['minute_mm_until'] === '04') or
				($_POST['minute_mm_until'] === '05') or
				($_POST['minute_mm_until'] === '06') or
				($_POST['minute_mm_until'] === '07') or
				($_POST['minute_mm_until'] === '08') or
				($_POST['minute_mm_until'] === '09'))


			or


			( ctype_digit($_POST['minute_mm_until']) && $_POST['minute_mm_until'] > 9 && $_POST['minute_mm_until'] < 60)) {

			update_post_meta( $post_id, 'utradiusdate_minute_until', $_POST['minute_mm_until'] );
			...
			update_post_meta( $post_id, 'utradiusdate_minute_until', 'mm' );
		...
		update_post_meta( $post_id, 'utradiusdate_hour_until', '' );
		update_post_meta( $post_id, 'utradiusdate_minute_until', '' );

	}
	
	
	
	$firstmonth = get_post_meta( $post_id, 'utradiusdate_month', true);
	$firstday = get_post_meta( $post_id, 'utradiusdate_day', true);
	$firstyear = get_post_meta( $post_id, 'utradiusdate_year', true);
	$firsthour = get_post_meta( $post_id, 'utradiusdate_hour', true);
	$firstminute = get_post_meta( $post_id, 'utradiusdate_minute', true);
	
	if ($firstmonth != "" && $firstday != "" && $firstyear != "" && $firsthour != "" && $firsthour != "hh"  && $firstminute != "" && $firstminute != "mm") {
		
	$firstdate = $firstday . "-" . $firstmonth . "-" . $firstyear . " " . $firsthour . ":" . $firstminute;	
		
	}
	
	elseif ($firstmonth != "" && $firstday != "" && $firstyear != "" && ($firsthour == "" || $firsthour == "hh") && ($firstminute == "" || $firstminute == "mm")) {
	...
	$firstdate_in_string = strtotime($firstdate);
	
	update_post_meta($post_id, 'firstdate', $firstdate);
	
	update_post_meta($post_id, 'firstdate_in_string', $firstdate_in_string);
	
	
	
	$lastmonth = get_post_meta( $post_id, 'utradiusdate_month_until', true);
	$lastday = get_post_meta( $post_id, 'utradiusdate_day_until', true);
	$lastyear = get_post_meta( $post_id, 'utradiusdate_year_until', true);
	$lasthour = get_post_meta( $post_id, 'utradiusdate_hour_until', true);
	$lastminute = get_post_meta( $post_id, 'utradiusdate_minute_until', true);
	
	if ($lastmonth != "" && $lastday != "" && $lastyear != "" && $lasthour != "" && $lasthour != "hh"  && $lastminute != "" && $lastminute != "mm") {
		
	$lastdate = $lastday . "-" . $lastmonth . "-" . $lastyear . " " . $lasthour . ":" . $lastminute;	
		
	}
	
	elseif ($lastmonth != "" && $lastday != "" && $lastyear != "" && ($lasthour == "" || $lasthour == "hh")  && ($lastminute == "" || $lastminute == "mm")) {
		
	$lastdate = $lastday . "-" . $lastmonth . "-" . $lastyear;	
		
	}
	
	$lastdate_in_string = strtotime($lastdate);
	
	
	update_post_meta($post_id, 'lastdate', $lastdate);
	
	update_post_meta($post_id, 'lastdate_in_string', $lastdate_in_string);
...
// Hook into the 'init' action
add_action( 'init', 'ut_writer_page_by', 0 );
...
function column_order($columnsd) {
  $n_columns = array();
  $move = 'taxonomy-section'; // what to move
  $before = 'categories'; // move before this
  foreach($columnsd as $key => $value) {
    if ($key==$before){
      $n_columns[$move] = $move;
    }
      $n_columns[$key] = $value;
...
function so_25737839( $columnsw ) 
{
    $columnsw['author'] = 'Post Editor';
    return $columnsw;
}


add_filter( 'manage_edit-post_columns', 'so_25737838' );

function so_25737838( $columnsq ) 
{
    $columnsq['title'] = 'Headline';
    return $columnsq;
}

add_filter( 'manage_edit-post_columns', 'so_25737834' );

function so_25737834( $columnsl ) 
{
    $columnsl['subtitle'] = 'Sub-Heading';
    return $columnsl;
}
...
transition-timing-function: ease-in-out, ease-in-out, ease-in-out;
 ...
 .fakedivlink2 {
      ...
      display: block; margin: 5px 0px 10px 0px; font-weight: 600; font-size: 13px; color: rgb(0, 115, 170); text-decoration: underline;
...
function utpostimageurl_meta_box() {
	...
	add_meta_box( 'tagsdiv-utpostimage', 'Post Image', 'utpostimage_meta_box', 'ut_radius_event_type', 'side', 'high' );
...
?><?php
	...
	add_action( 'wp_ajax_ajax_order_more', 'my_ajax_order_more');
...
function my_ajax_order_more() {
	...
	$args = array(
		...
		'paged' => $ajax_page_no,
		'post__not_in' =>  $current_post_order_list_option,
	
		);
			
	$the_query = new WP_Query( $args ); 
	$postnumber = (($ajax_page_no - 1)*10);
	...
	if ( $the_query->have_posts() ) : 
		...
		?>
			...
			<tr>
				...
				<td style="padding-bottom: 18px;"><?php echo $postnumber; ?></td>
				<td><?php the_title(); ?> (<?php echo get_the_ID(); ?>)</td>
			
				<td>
					<input style="height: 28px;" name="positionrequest" type="text" size="2" value="" />
	...
	add_action( 'wp_ajax_ajax_clear_order', 'my_ajax_clear_order');
...
// Begin UT Post Order AJAX Function
...
function my_ajax_postorder_form_submit() {
	...
	$post_order_form_data = $_POST['form'];
	$exploded_post_order_query = explode ( "&" , $post_order_form_data);
	
	$posts_with_pos = array();
	// Iterate through and construct an array with positions of articles : ids
	for ($x = 0; $x <= count($exploded_post_order_query); $x=$x+2) {
				
	    $pos_array = explode( "=" , $exploded_post_order_query[$x]);
	    $pos = $pos_array[1];
	    $post_id_array = explode( "=" , $exploded_post_order_query[$x+1]);
	    $post_id = $post_id_array[1];
	
	    if($pos != "" && intval($pos)>0 && intval($pos)<=25){
		   $post_with_pos[intval($pos)] = $post_id;
		}
	}
	$current_post_order_list = get_option("ut_post_order_list_3");
	if(!empty($post_with_pos)){
		
		ksort($post_with_pos);
		$current_post_order_list = postorder_update($post_with_pos, $current_post_order_list);

	}
	//write that current list
	
	update_option("ut_post_order_list_3", $current_post_order_list);
	
	print_r($post_with_pos);
	print_r($post_order_form_data);
	echo "<br/><br/><br/>";
	print_r($exploded_post_order_query);
	
	echo "<h4>";
	
	print_r($current_post_order_list);
...
function postorder_update($post_order_with_post_id, $current_post_order_list) {
	
 	if (empty($current_post_order_list)) {
		$current_post_order_list = array();
 	}
	for ($i = 0; $i < count($post_order_with_post_id); $i++) {
		
		$post_position = key($post_order_with_post_id);
		$post_id = $post_order_with_post_id[$post_position];
		$current_post_length = count($current_post_order_list);
		
		if(($key = array_search($post_id, $current_post_order_list)) !== false) {
			unset($current_post_order_list[$key]);
		}	
		
		if ( $post_position > $current_post_length) {
			$current_post_order_list[] = $post_id;
		}
		
		else {
			array_splice($current_post_order_list, $post_position-1, 0, $post_id);
		}
		next($post_order_with_post_id);	
	}

	return($current_post_order_list);
...
// Begin UT Post Order Page Thing


add_action('admin_menu', 'add_global_custom_options_order');

function add_global_custom_options_order()
{

	$page_title = 'UT Post Order';
	$menu_title = 'UT Post Order';
	$capability = 'manage_options';
	$menu_slug = 'utpostorder';
	$function = 'ut_post_order_options';
	$icon_url = '';
	$position = '8';
...
add_action( 'admin_menu', 'ut_post_order' );

/** Step 1. */
function ut_post_order() {
	add_options_page( 'UT Post Order', 'UT Post Order', 'manage_options', 'ut-post-order', 'ut_post_order_options' );
}
	...
	<?php 
		...
		$args = array(
			...
			'post_status' => 'publish',
				...
				'post__in' =>  $current_post_order_list_option, );
...
<div style="font-weight: normal; margin: 20px 0px 15px 0px; font-size: 15px;"><input class="button button-primary" name="updatepostorder" id="updatepostorder" type="submit" value="Update Post Order" style="margin: 8px 7px 0px 0px;"/><input class="button" name="clearpostorder" id="clearpostorder" type="submit" value="Clear Ordered Posts" style="margin-top: 8px;"/></div>

<div id="leftlistposts">

<h3>Ordered Posts</h3>


<?php if ( $the_query->have_posts() && $current_post_order_list_option != "" ) : ?>




<form id="current_orderpostlist">


<table id="listofposts" class="wp-list-table widefat striped" style="font-size: 15px;">
	...
	<tr >
		...
		<td><?php the_title(); ?> (<?php echo get_the_ID(); ?>)
		...
		<td>
			...
			<input style="height: 28px; float: left;" name="positionrequest_inorder" type="text" size="2" value="" />
<input type="hidden" size="3" name="postID_inorder" value="<?php echo get_the_ID(); ?>" />
...
<table id="listofposts" class="wp-list-table widefat striped" style="font-size: 15px;">
	...
	<tr>
		...
		<td>There are no ordered posts.	</td>
...
<div id="rightlistposts">



<h3>Unordered Posts</h3>
...
<table id="listofposts" class="wp-list-table widefat striped" style="font-size: 15px;">
	...
	<tr >
		...
		<td>
			...
			<input style="height: 28px; float: left;" name="positionrequest" type="text" size="2" value="" />
<input type="hidden" size="3" name="postID" value="<?php echo get_the_ID(); ?>" />
...
<div id="loadmoreposts" class="fakedivlink2" style="background: none; vertical-align: middle;" href="#">Load More Posts
	
	<div id="loadmoreeventsgif" style="display: inline-block; visibility:hidden; padding: 0px 0px 0px 5px; vertical-align: middle;"><img src="<?php echo admin_url(); ?>/images/wpspin_light.gif" /></div>
...
<div id="STUFFTOPRINT">
 ...
 jQuery( "#clearpostorder" ).click(function() {
 ...
 jQuery( "#updatepostorder" ).click(function(e) {
   ...
   jQuery.ajax({
    ...
    success: function( html ) {
			...
			jQuery('#STUFFTOPRINT').append( html );
...
<?php
	...
	add_action( 'wp_ajax_ajax_order_more_freshers', 'my_ajax_order_more_freshers');
...
function my_ajax_order_more_freshers() {
	...
	$args = array(
		...
		'posts_per_page' => $postsperpage,
		'paged' => $ajax_page_no,
		'post__not_in' =>  $current_post_order_freshers_list_option,
	...
	add_action( 'wp_ajax_ajax_clear_order_freshers', 'my_ajax_clear_order_freshers');
...
// Begin Freshers Post Order AJAX Function
...
function my_ajax_postorder_freshers_form_submit() {
	...
	$post_order_freshers_form_data = $_POST['form'];
	$exploded_post_order_freshers_query = explode ( "&" , $post_order_freshers_form_data);
	
	$posts_with_pos = array();
	// Iterate through and construct an array with positions of articles : ids
	for ($x = 0; $x <= count($exploded_post_order_freshers_query); $x=$x+2) {
				
	    $pos_array = explode( "=" , $exploded_post_order_freshers_query[$x]);
	    $pos = $pos_array[1];
	    $post_id_array = explode( "=" , $exploded_post_order_freshers_query[$x+1]);
	    $post_id = $post_id_array[1];
	...
	$current_post_order_freshers_list = get_option("ut_post_order_freshers_list_3");
	if(!empty($post_with_pos)){
		
		ksort($post_with_pos);
		$current_post_order_freshers_list = postorder_freshers_update($post_with_pos, $current_post_order_freshers_list);

	}
	//write that current list
	
	update_option("ut_post_order_freshers_list_3", $current_post_order_freshers_list);
	
	print_r($post_with_pos);
	print_r($post_order_freshers_form_data);
	echo "<br/><br/><br/>";
	print_r($exploded_post_order_freshers_query);
	
	echo "<h4>";
	
	print_r($current_post_order_freshers_list);
...
function postorder_freshers_update($post_order_freshers_with_post_id, $current_post_order_freshers_list) {
	
 	if (empty($current_post_order_freshers_list)) {
		$current_post_order_freshers_list = array();
 	}
	for ($i = 0; $i < count($post_order_freshers_with_post_id); $i++) {
		
		$post_position = key($post_order_freshers_with_post_id);
		$post_id = $post_order_freshers_with_post_id[$post_position];
		$current_post_length = count($current_post_order_freshers_list);
		
		if(($key = array_search($post_id, $current_post_order_freshers_list)) !== false) {
			unset($current_post_order_freshers_list[$key]);
		}	
		
		if ( $post_position > $current_post_length) {
			$current_post_order_freshers_list[] = $post_id;
		}
		
		else {
			array_splice($current_post_order_freshers_list, $post_position-1, 0, $post_id);
		}
		next($post_order_freshers_with_post_id);	
	}

	return($current_post_order_freshers_list);
...
// Begin Freshers Post Order Page Thing


add_action('admin_menu', 'add_global_custom_options_order_freshers');

function add_global_custom_options_order_freshers()
{

	$page_title = 'Freshers Post Order';
	$menu_title = 'Freshers Post Order';
	$capability = 'manage_options';
	$menu_slug = 'utfresherspostorder';
	$function = 'ut_post_order_freshers_options';
	$icon_url = '';
	$position = '8';
...
?><?php
/** Step 2 (from text above). */
add_action( 'admin_menu', 'ut_post_order_freshers' );

/** Step 1. */
function ut_post_order_freshers() {
	add_options_page( 'Freshers Post Order', 'Freshers Post Order', 'manage_options', 'ut-post-order', 'ut_post_order_freshers_options' );
}
	...
	<?php 
		...
		$args = array(
			...
			'post_status' => 'publish',
				...
				'post__in' =>  $current_post_order_freshers_list_option, );
...
<?php if ( $the_query->have_posts() && $current_post_order_freshers_list_option != "" ) : ?>
