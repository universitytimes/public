<?php get_header(); ?>
			...
			<div id="content">
				...
				<div id="articlecontent">
						...
						<?php if ( is_object_in_term( $post->ID, 'section', 'opinion' ) && is_object_in_term( $post->ID, 'articletype', 'editorials' ) && in_category('patrick-prendergast-legacy') ) : ?>
					
                 <?php include(locate_template('provosteditorialtemplate.php'));   // Editorial Template (post-editorial.php) ?> 

		
<?php elseif ( is_object_in_term( $post->ID, 'section', 'opinion' ) && is_object_in_term( $post->ID, 'articletype', 'editorials' ) ) : ?>
...
<?php elseif ( is_object_in_term( $post->ID, 'section', 'freshers' ) ) : ?>
								
				<?php get_template_part( 'freshersarticlestemplate' );  // Your Essential College Guide Articles (freshersarticlestemplate.php) ?>	
				
				
				
				
<?php elseif ( is_object_in_term( $post->ID, 'section', 'radius' ) ) : ?>
								
				<?php get_template_part( 'magazinetemplate' );  // AS OF JUN 2026 WE ARE USING THE OLD MAGAZINE FEATURE POST TEMPLATE FOR ALL RADIUS ARTICLES INSTEAD OF THE REGULAR RADIUS TEMPLATE (radiusposttemplate.php). I can't figure out how to create a new file for an updated radius template and I don't want to delete the old one in case someone wants to use it again in the future so leaving it like this for now... sorry... ?>				
...
<?php else : ?>
                  ...
                  <h2 class="articleheadline" itemprop="headline"><?php the_title(); ?></h2>
