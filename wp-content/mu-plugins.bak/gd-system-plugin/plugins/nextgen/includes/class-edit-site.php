...
class Edit_Site {
	...
	public function enqueue_scripts() {
		...
		$asset_filepath = GD_NEXTGEN_PLUGIN_DIR . '/build/editSite.asset.php';
		$asset_file     = file_exists( $asset_filepath ) ? include $asset_filepath : $default_asset_file;

		wp_enqueue_script(
			'nextgen-edit-site',
			GD_NEXTGEN_PLUGIN_URL . 'build/editSite.js',
		...
		wp_enqueue_style(
			'nextgen-edit-site',
			GD_NEXTGEN_PLUGIN_URL . 'build/style-editSite.css',
			[],
			$asset_file['version']
		);
	...
	public function initialize_nextgen_block_editor( $tag, $handle ) {
		...
		if ( 'wp-edit-post' === $handle ) {
			$tag = str_replace(
				...
				'nextgen.editSite.initializeRender',
				$tag
			);
		}
