...
class PaymentForm
{
    ...
    public function enqueueStyles()
    {
        Enqueue::style()
            ...
            ->setSource(WordPressRepository::getAssetsUrl('css/payments/payment-form.css'))
            ->execute();
    }
    ...
    protected function getPaymentMethods() : array
    {
        ...
        foreach ($this->getWooCommerceTokens() as $wooCommerceToken) {
            try {
                ...
                $paymentMethod = (new PaymentMethodDataStore($this->providerName))->read($wooCommerceToken->get_id());
    ...
    protected function renderPaymentMethods()
    {
        ...
        if (is_subclass_of($view, AbstractPaymentMethodView::class)) {
            (new $view($paymentMethod))->render($paymentMethod->getId() === $this->defaultPaymentMethodId);
        } ?>
                <br>
