...
abstract class AbstractPaymentGateway extends WC_Payment_Gateway
{
     ...
     * @throws Exception
     */
    protected function getTransactionForPayment(WC_Order $order) : PaymentTransaction
    {
     ...
     * @throws Exception
     */
    protected function getTransactionForVoid(WC_Order $order) : VoidTransaction
    {
     ...
     * @return mixed
     */
    public function maybeSetOrderReceivedText($text)
    {
        if (WC()->session && $newText = (string) WC()->session->order_received_text) {
            $text = $newText;
    ...
    protected function maybeSetRemoteParentId(WC_Order $order, AbstractTransaction $transaction)
    {
        try {
            ...
            $orderTransactionDataStore = new $binding($this->providerName);
    ...
    {
        $customerMessage = (new PaymentTransactionMessageAdapter($transaction))->convertFromSource();
