...
final class File_Editor {
	...
	private function is_file_editor_request() {
		...
		return ( $this->is_file_editor() && isset( $_POST['action'] ) && 'update' === $_POST['action'] ); // @codingStandardsIgnoreLine
	...
	public function file_edit() {
		...
		wp_die( esc_html__( 'File editing is not enabled on this site.', 'gd-system-plugin' ) );
	...
	public function display_warning() {
		...
		<script type="text/template" id="wpaas-file-editor-disabled-message">
			<h1><?php esc_html_e( 'File Editor', 'gd-system-plugin' ); ?></h1>
			<p><?php esc_html_e( 'For your security, the WordPress file editor is disabled by default.', 'gd-system-plugin' ); ?> <a href="https://www.godaddy.com/help/managed-wordpress-file-editing-limitations-8943"><?php esc_html_e( 'Learn More', 'gd-system-plugin' ); ?></a></p>
			<p><?php esc_html_e( 'If you enable file editing all plugin and theme files on the server will become editable.', 'gd-system-plugin' ); ?></p>
			<p>
				<label for="wpaas-file-editor-verify">
					<input type="checkbox" id="wpaas-file-editor-verify" value="" />
					<?php esc_html_e( 'I understand the risks involved with enabling this feature', 'gd-system-plugin' ); ?>
				</label>
			</p>
			<p><a href="<?php echo esc_url( self::url() ); ?>" id="wpaas-file-editor-enable" class="button button-primary button-primary-disabled"><?php esc_html_e( 'Enable File Editing', 'gd-system-plugin' ); ?></a></p>
		</script>
		<?php
