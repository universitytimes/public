...
/*
Plugin Name: Smash Balloon X (Twitter) Feed
...
function ctf_maybe_shorten_text( $string, $feed_settings ) {
	...
	$return = implode( ' ', $first_parts ) . '<button type="button" class="ctf_more" aria-expanded="false" aria-label="' . esc_attr__( 'Show full tweet text', 'custom-twitter-feeds' ) . '" style="background:none;border:0;padding:0;font:inherit;color:inherit;cursor:pointer;"><span aria-hidden="true">...</span></button><span class="ctf_remaining">';
...
function ctf_replace_urls( $string, $feed_settings, $post ) {
	...
	if ( isset( $post['entities']['urls'][0] ) ) {
		foreach ( $post['entities']['urls'] as $url ) {
		    if ( isset( $url['url'] ) ) {
			    // The tweet text is already esc_html()'d before this filter runs, so the
			    // replacement must be encoded the same way or it re-opens a raw channel
			    // into the escaped string (SMASH-1796).
			    $string = str_replace( $url['url'], esc_html( $url['expanded_url'] ), $string );
		    }
		}
	}
...
function ctf_get_fa_el( $icon ) {
	...
	$elems = array(
		...
		'fa-heart' => array(
			...
			'svg' => '<svg class="svg-inline--fa fa-w-16" viewBox="0 0 24 24" aria-hidden="true" focusable="false" xmlns="http://www.w3.org/2000/svg"><g><path fill="currentColor" d="M12 21.638h-.014C9.403 21.59 1.95 14.856 1.95 8.478c0-3.064 2.525-5.754 5.403-5.754 2.29 0 3.83 1.58 4.646 2.73.814-1.148 2.354-2.73 4.645-2.73 2.88 0 5.404 2.69 5.404 5.755 0 6.376-7.454 13.11-10.037 13.157H12zM7.354 4.225c-2.08 0-3.903 1.988-3.903 4.255 0 5.74 7.034 11.596 8.55 11.658 1.518-.062 8.55-5.917 8.55-11.658 0-2.267-1.823-4.255-3.903-4.255-2.528 0-3.94 2.936-3.952 2.965-.23.562-1.156.562-1.387 0-.014-.03-1.425-2.965-3.954-2.965z"></path></g></svg>'
		),
		'fa-twitter' => array(
			'icon' => '<span class="fa fab fa-twitter"></span>',
...
function ctf_scripts_and_styles( $enqueue = false ) {
	...
	$loacalize_args = array(
		...
		'i18n'     => array(
			/* translators: %s: number of new tweets loaded into the feed (screen reader announcement) */
			'newTweetsLoaded' => __( '%s new tweets loaded', 'custom-twitter-feeds' ),
			'noMoreTweets'    => __( "That's all! No more Tweets to load", 'custom-twitter-feeds' ),
		),
	);
...
function ctf_admin_scripts_and_styles() {
	// SMASH-1378: design-token mirror (provides --sb-focus-ring etc.) registered
	// as a dependency of the admin stylesheets so the focus-ring custom property
	// is available across all TW Free admin pages. Also registered in
	// CTF_Feed_Builder::global_enqueue_ressources_scripts() for the builder/
	// settings lifecycle; wp_register_style() is idempotent.
	wp_register_style( 'ctf-tokens-local', CTF_PLUGIN_URL . 'assets/tokens/ctf-tokens-local.css', array(), CTF_VERSION );
	wp_enqueue_style(
		'feed-global-style',
    ...
    wp_enqueue_style( 'ctf_admin_styles', plugins_url( '/css/ctf-admin-styles.css', __FILE__ ), array( 'ctf-tokens-local' ), CTF_VERSION );
...
add_action( 'init', 'ctf_text_domain' );
