<div class="ctf-fb-sourcelist-pp-ctn sb-fs-boss ctf-fb-center-boss" v-if="viewsActive.sourcesListPopup" data-source="active" role="dialog" aria-modal="true" :aria-label="genericText.dialogSources" @keydown.esc="toggleFeedTypesSourcesPopup()">
	<div class="ctf-fb-source-popup ctf-fb-popup-inside ctf-fb-source-pp-customizer">
		<div class="ctf-fb-popup-cls" role="button" tabindex="0" :aria-label="genericText.close" @click.prevent.default="toggleFeedTypesSourcesPopup()" @keydown.enter.prevent="toggleFeedTypesSourcesPopup()" @keydown.space.prevent="toggleFeedTypesSourcesPopup()">
		...
		<div class="ctf-fb-sourcelist-pp">
			<div class="ctf-fb-srcslist-ctn ctf-fb-fs" role="group" :aria-label="genericText.sourcesList" v-if="sourcesList.length > 0">
				<div class="ctf-fb-srcs-item ctf-fb-srcs-new" role="button" tabindex="0" :aria-label="genericText.addNew" @click.prevent.default="activateView('sourcePopup', 'creationRedirect')" @keydown.enter.prevent="activateView('sourcePopup', 'creationRedirect')" @keydown.space.prevent="activateView('sourcePopup', 'creationRedirect')">
				...
				<div class="ctf-fb-srcs-item ctf-fb-onbrd-tltp-parent ctf-fb-onbrd-tltp-center-top ctf-fb-onbrd-tltp-hover" v-for="(source, sourceIndex) in sourcesList" role="button" :tabindex="checkSourceDisabledPopup(source) ? -1 : 0" :aria-disabled="checkSourceDisabledPopup(source) ? 'true' : null" :aria-label="source.username || source.name" @click.prevent.default="selectSourcePopup(source)" @keydown.enter.prevent="selectSourcePopup(source)" @keydown.space.prevent="selectSourcePopup(source)" :data-type="source.account_type" :data-active="isSourceSelectActivePopup(source)"  :data-disabled="checkSourceDisabledPopup(source)">
					...
					<div class="ctf-fb-srcs-item-inf">
						...
						<div class="ctf-fb-left-boss">
							...
							<div v-if="source.error !== ''" class="sb-source-error-wrap">
								...
								<span v-html="genericText.errorSource"></span><button type="button" @click.prevent.default="activateView('sourcePopupType', 'creation')" v-html="genericText.reconnect"></button>
							</div>
