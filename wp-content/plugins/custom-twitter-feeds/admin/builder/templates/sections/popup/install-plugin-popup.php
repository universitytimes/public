<script type="text/x-template" id="install-plugin-popup">
    <div class="ctf-fb-source-ctn sb-fs-boss ctf-fb-center-boss" role="dialog" aria-modal="true" aria-label="<?php esc_attr_e('Install plugin', 'custom-twitter-feeds'); ?>" @keydown.esc.prevent="$parent.activateView('installPluginPopup')" v-if="viewsActive.installPluginPopup">
        <div class="ctf-fb-source-popup ctf-fb-popup-inside ctf-install-plugin-modal">
            <div class="ctf-fb-popup-cls" role="button" tabindex="0" :aria-label="genericText.close" @click.prevent.default="$parent.activateView('installPluginPopup')" @keydown.enter.prevent="$parent.activateView('installPluginPopup')" @keydown.space.prevent="$parent.activateView('installPluginPopup')">
