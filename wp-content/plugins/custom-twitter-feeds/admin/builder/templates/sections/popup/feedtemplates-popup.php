<div class="ctf-fb-feedtypes-pp-ctn ctf-fb-feedtemplates-ctn sb-fs-boss ctf-fb-center-boss" v-if="viewsActive.feedtemplatesPopup" role="dialog" aria-modal="true" :aria-label="genericText.dialogFeedTemplates">
	<div class="ctf-fb-feedtypes-popup ctf-fb-popup-inside">
		<div class="ctf-fb-popup-cls" role="button" tabindex="0" :aria-label="genericText.close" @click.prevent.default="activateView('feedtemplatesPopup')" @keydown.enter.prevent="activateView('feedtemplatesPopup')" @keydown.space.prevent="activateView('feedtemplatesPopup')"><svg aria-hidden="true" focusable="false" width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="ctf-fb-source-top ctf-fb-fs">
            ...
            <div class="ctf-fb-types ctf-fb-fs">
                <div class="ctf-fb-templates-list ctf-fb-templates-ctn">
                    <div :class="['ctf-fb-type-el', 'ctf-feed-template-' + feedTemplateEl.type]" v-for="(feedTemplateEl, feedTemplateIn) in feedTemplates" role="button" tabindex="0" :aria-label="feedTemplateEl.title" :aria-pressed="choosedFeedTemplateCustomizer(feedTemplateEl.type) ? 'true' : 'false'" :data-active="choosedFeedTemplateCustomizer(feedTemplateEl.type)" @click.prevent.default="viewsActive.extensionsPopupElement = 'feedTemplates'; activateView('feedtemplatesPopup')" @keydown.enter.prevent="viewsActive.extensionsPopupElement = 'feedTemplates'; activateView('feedtemplatesPopup')" @keydown.space.prevent="viewsActive.extensionsPopupElement = 'feedTemplates'; activateView('feedtemplatesPopup')">
