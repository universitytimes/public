...
<div v-for="tooltip in customizeScreensText.onboarding.tooltips"
     ...
     role="dialog" aria-modal="true" :aria-label="genericText.dialogOnboarding" v-bind:aria-labelledby="'sb-onboarding-tooltip-' + customizeScreensText.onboarding.type + '-' + tooltip.step + '-heading'" @keydown.esc.prevent="onboardingClose()">
	<div class="ctf-fb-popup-cls" role="button" tabindex="0" :aria-label="genericText.close" @click.prevent.default="onboardingClose()" @keydown.enter.prevent="onboardingClose()" @keydown.space.prevent="onboardingClose()">
	...
	<div class="sb-onboarding-top-row">
		<h2 tabindex="-1" v-bind:id="'sb-onboarding-tooltip-' + customizeScreensText.onboarding.type + '-' + tooltip.step + '-heading'" class="sb-standard-p sb-bold sb-onboarding-wizard-step-heading">{{tooltip.heading}}</h2>
	...
	<div class="sb-pointer" v-bind:class="{ 'sb-bottom-pointer' : typeof tooltip.pointer === 'undefined' || tooltip.pointer === 'bottom' }">
		<svg aria-hidden="true" focusable="false" v-if="typeof tooltip.pointer !== 'undefined' && tooltip.pointer === 'top'" width="20" height="10" viewBox="0 0 20 10" fill="none" xmlns="http://www.w3.org/2000/svg">
			<path d="M8.58578 1.41421C9.36683 0.633166 10.6332 0.633165 11.4142 1.41421L20 10H0L8.58578 1.41421Z" fill="white"/>
		</svg>
		<svg aria-hidden="true" focusable="false" v-else width="20" height="10" viewBox="0 0 20 10" fill="none" xmlns="http://www.w3.org/2000/svg">
