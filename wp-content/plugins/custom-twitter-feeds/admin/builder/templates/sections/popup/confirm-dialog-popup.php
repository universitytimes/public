<script type="text/x-template" id="sb-confirm-dialog-component">
	<div class="sb-dialog-ctn sb-fs-boss ctf-fb-center-boss" v-if="dialogBoxElement.active" role="dialog" aria-modal="true" :aria-label="genericText.dialogConfirmAction">
		<div class="sb-dialog-popup ctf-fb-popup-inside">
			<div class="ctf-fb-popup-cls" role="button" tabindex="0" :aria-label="genericText.close" @click.prevent.default="closeConfirmDialog" @keydown.enter.prevent="closeConfirmDialog" @keydown.space.prevent="closeConfirmDialog"><svg aria-hidden="true" focusable="false" width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
			...
			<div class="sb-dialog-remove-source ctf-fb-fs" v-if="dialogBoxElement.type == 'deleteSourceCustomizer'">
				<div class="ctf-fb-srcs-item" :data-type="sourceToDelete.account_type">
					<div class="ctf-fb-srcs-item-avatar" v-if="returnAccountAvatar(sourceToDelete)">
						<img :src="returnAccountAvatar(sourceToDelete)" :alt="sourceToDelete.username ? sourceToDelete.username + ' avatar' : ''">
					</div>
