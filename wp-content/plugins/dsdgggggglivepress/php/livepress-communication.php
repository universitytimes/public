...
class LivePress_Communication {
	...
	public function get_blog($post_vars=null) {
		...
		$cache_id = 'lp_blog_details';

		$cache = get_transient( $cache_id );
		if( false === $cache ) {
			try {
				$cache = json_decode( $this->request_content_from_livepress( '/blog/get', 'post', $post_vars ) )->blog;
				set_transient( $cache_id, $cache,  MINUTE_IN_SECONDS * 5 );
			} catch ( Exception $e ) {
				return '';
			}
		}
	...
	public function validate_on_livepress( $domains ) {
		...
		delete_transient( self::$blog_cache_id );
