...
require_once( LP_PLUGIN_PATH . 'php/livepress-json-ld.php' );
require_once( LP_PLUGIN_PATH . 'php/SimpleScraper.class.php');
require_once( LP_PLUGIN_PATH . 'php/livepress-pause-updates.php');
...
add_action( 'after_setup_theme', 'LivePress_Compatibility_Fixes::instance', 100 );
...
add_action( 'wp_ajax_lp_update-live-status-to-finished',   array( $blogging_tools, 'toggle_live_status_to_finished' ) );
add_action( 'wp_ajax_lp_update-live-status-to-live',   array( $blogging_tools, 'toggle_live_status_to_live' ) );

add_filter( 'livepress_update_numberposts', function() { return 30; } );
...
foreach ( apply_filters( 'livepress_post_types', array( 'post' ) ) as $post_types ) {
	add_filter( "manage_{$post_types}s_columns" , 'add_livepress_status_column' );
	add_action( "manage_{$post_types}s_custom_column" , array( $blogging_tools, 'display_posts_livestatus' ) , 10, 2 );
}
