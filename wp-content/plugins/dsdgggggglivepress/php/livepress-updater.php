...
class LivePress_Updater {
	...
	function __construct() {
		...
		register_taxonomy(
			...
			array(
				'label'   => __( 'Live Update Tags', 'livepress' ),
				'public'  => false,
				'show_ui' => true,
			)
		);
	...
	function add_css_and_js_on_header( $hook = null ) {
		...
		if ( true === apply_filters( 'livepress_dont_load_js_on_homepage', is_home() ) ) {

			return;
		}

		if ( '' !== $hook && null !== $hook && 'post-new.php' !== $hook && 'post.php' !== $hook && 'settings_page_livepress-settings' !== $hook ) {
		...
		if ( isset( $post ) && ! is_admin() ) {
			...
			$is_live        = apply_filters( 'livepress_add_css_and_js_on_header', $blogging_tools->get_post_live_status( $post->ID ) );
			if ( ! $is_live ) {
		...
		if ( is_page() || is_single() || is_home() ) {
			...
			// Add datetimepicker for Json+LD
			wp_enqueue_style( 'anytime', LP_PLUGIN_URL . 'css/anytime.5.1.2.css', array(), LP_PLUGIN_VERSION );
	...
	function add_js_config( $hook = null ) {
		...
		if ( is_admin() && null !== $hook && 'post-new.php' !== $hook && 'post.php' !== $hook && 'settings_page_livepress-settings' !== $hook ) {
		...
		$is_live = false;
		if ( null !== $post ) {
			$current_post_id = apply_filters( 'livepress_current_post_id', $post->ID );
			$is_live         = isset( $post ) ? LivePress_Updater::instance()->blogging_tools->get_post_live_status( $current_post_id ) : false;
		}
		...
		if ( null !== $post ) {
			...
			$ljsc->new_value( 'post_finished', ( $finished ) ? 'true' : 'false' );
		}
		...
		$ljsc->new_value( 'page_type', apply_filters( 'livepress_js_config_page_type', $page_type ) );

		// Comments
		if ( isset( $current_post_id ) ) {
			$args                          = array( 'post_id' => ( isset( $current_post_id ) ? $current_post_id : null ) );
		...
		$ljsc->new_value( 'oortle_diff_inserted', apply_filters( 'livepress_effects_inserted', '#eaeaea' ) );
		$ljsc->new_value( 'oortle_diff_changed', apply_filters( 'livepress_effects_changed', '#eaeaea' ) );
		...
		if ( is_admin() || $is_live ) {
			if ( isset( $current_post_id ) && $current_post_id ) {
				$args          = array( 'post_id' => $current_post_id );
				$post_comments = get_comments( $args );
				...
				$ljsc->new_value( 'post_id', $current_post_id );
				...
				$livepress_live_status = get_post_meta( $current_post_id, '_livepress_live_status', true );
				if ( isset( $livepress_live_status['last_post'] ) ) {
					$ljsc->new_value( 'last_post', $livepress_live_status['last_post'] );
				}
			...
			if ( is_admin() ) {
				...
				$ljsc->new_value( 'PostMetainfo', '', Livepress_Configuration_Item::$BLOCK ); // open PostMetainfo
					...
					$ljsc->new_value( 'gmt_offset', get_option( 'gmt_offset' ) );
					$ljsc->new_value( 'gmt_offset_in_minutes', get_option( 'gmt_offset' ) * 60 );
					$timezone_string_option = get_option( 'timezone_string' );
					$timezone_string = ( ! empty( $timezone_string_option ) && false !== $timezone_string_option ) ? $timezone_string_option : 'Europe/London';
					$ljsc->new_value( 'timezone_string', $timezone_string );
					$timezone_string_date = new DateTime( '2000-01-01', new DateTimeZone( $timezone_string ) );
					$ljsc->new_value( 'timezone_string_short', $timezone_string_date->format( 'T' ) );
				...
				$ljsc->new_value( 'PostMetainfo', '', Livepress_Configuration_Item::$ENDBLOCK ); // close PostMetainfo
		...
		$ljsc->new_value( 'disable_comments', apply_filters( 'LP_disable_ajax_comments', 'true'  ) );
		...
		if ( isset( $settings['timestamp_format'] ) ) {
		...
		/*
		 * allows you to change the target for the scroll into veiw
		 *
		 *  Since 1.3.9
		 *
		 * $CSS_selector string
		 */
		$ljsc->new_value( 'scroll_target_for_post_title',  apply_filters( 'livepress_scroll_target_for_post_title_selector', '.entry-title' ) );
		/*
		 * allows you to control if the updates auto show when you scroll them into view
		 *
		 *  Since 1.3.9
		 *
		 * $CSS_selector string
		 */
		$ljsc->new_value( 'show_update_on_scroll',  apply_filters( 'livepress_show_update_on_scroll', 'true' ) );

		$ljsc->new_value( 'new_updates', esc_html__( 'new updates', 'livepress' ) );
		$ljsc->new_value( 'new_update', esc_html__( 'new update', 'livepress' ) );
		// translator note ##count## is the nunmber of updates
		$ljsc->new_value( 'view_new_updates', esc_html__( 'View ##count## new updates', 'livepress' ) );
		// translator note ##count## is the nunmber of updates
		$ljsc->new_value( 'view_new_update', esc_html__( 'View ##count## new update', 'livepress' ) );
	 ...
	 */
	function add_global_post_content_tag( $content, $post_modified_gmt = null, $livetags = array(), $classes = array() ) {
		global $post;
		...
		if ( $div_id ) {
			$content =  sprintf( '<div id="%s" class="%s"  data-livetags="%s">%s</div>',
				esc_attr( $div_id ),
				 implode(' ', array_merge( array('livepress_content'), $classes ) ),
				trim( $live_tag_data, ',' ),
				$content
			);
		}
