...
if ( !class_exists( 'MeowKit_WR2X_Admin' ) ) {
  ...
  class MeowKit_WR2X_Admin {
    ...
    public function __construct( $prefix, $mainfile, $domain, $isPro = false, $disableReview = false, $freeOnly = false ) {
      ...
      if ( is_admin() ) {
        ...
        if ( MeowKit_WR2X_Helpers::is_asynchronous_request() ) {
          return;
        }
    ...
    public static function process_deferred_instances() {
      ...
      if ( !function_exists( 'current_user_can' ) || !function_exists( 'wp_get_current_user' ) ) {
        trigger_error(
          'MeowKit_WR2X_Admin: Pluggable functions still unavailable on init hook. ' .
    ...
    private function run_admin_setup() {
      ...
      if ( !MeowKit_WR2X_Admin::$loaded ) {
        // Check for potential issues with WordPress install, other plugins, etc.
        new MeowKit_WR2X_Issues( $this->prefix, $this->mainfile, $this->domain );
      ...
      if ( $this->is_user_admin() ) {
        if ( !$this->disableReview ) {
          new MeowKit_WR2X_Ratings( $this->prefix, $this->mainfile, $this->domain );
        }
        new MeowKit_WR2X_News( $this->domain );
      }
    }
    ...
    public function meowapps_hide_dashboard_callback() {
      ...
      echo MeowKit_WR2X_Helpers::wp_kses( $html );
    }
